...
export const chapterLeaderService = {
  async getChapterStats(chapterId: string): Promise<{ data: ChapterStats | null; error: any }> {
    try {
      ...
      const stats: ChapterStats = {
        ...
        avgParticipation: currentTotals.participation > 0 ? Math.round((currentTotals.participation / (totalMembers || 1)) * 100) : 0,
  ...
  async getChapterMembers(chapterId: string, page: number = 1, limit: number = 20): Promise<{ data: ChapterMember[] | null; error: any; totalCount: number }> {
    try {
  ...
  async getChapterTrades(chapterId: string, page: number = 1, limit: number = 20, filters?: any): Promise<{ data: ChapterTrade[] | null; error: any; totalCount: number }> {
    try {
      const offset = (page - 1) * limit;
      ...
      const { data: trades, error, count } = await query.range(offset, offset + limit - 1);
      ...
      const tradesWithProfiles: ChapterTrade[] = (trades || []).map(trade => ({
        ...trade,
        user: profiles?.find(p => p.id === trade.user_id) || { full_name: 'Unknown', business_name: 'Unknown' },
        source_member: trade.source_member_id 
          ? profiles?.find(p => p.id === trade.source_member_id) || { full_name: 'Unknown', business_name: 'Unknown' }
          : undefined,
        beneficiary_member: trade.beneficiary_member_id 
          ? profiles?.find(p => p.id === trade.beneficiary_member_id) || { full_name: 'Unknown', business_name: 'Unknown' }
          : undefined,
      }));
  ...
  async getChapterActivity(chapterId: string, limit: number = 50): Promise<{ data: ChapterActivity[] | null; error: any }> {
    try {
      // Get recent metrics
      const { data: metrics } = await supabase
        .from('metrics')
      ...
      (metrics || []).forEach(metric => {
        const user = profiles?.find(p => p.id === metric.user_id) || { full_name: 'Unknown', business_name: 'Unknown' };
        activities.push({
          id: metric.id,
          type: 'metric',
          description: this.formatMetricActivity(metric.metric_type, metric.value),
      ...
      (trades || []).forEach(trade => {
        const user = profiles?.find(p => p.id === trade.user_id) || { full_name: 'Unknown', business_name: 'Unknown' };
        activities.push({
          id: trade.id,
          type: 'trade',
          description: `Recorded ${this.formatCurrency(trade.amount)} trade${trade.description ? ` - ${trade.description}` : ''}`,
  ...
  aggregateMetrics(metrics: any[]): { participation: number; learning: number; activity: number; networking: number; trade: number } {
    return metrics.reduce((acc, metric) => {
      acc[metric.metric_type] = (acc[metric.metric_type] || 0) + Number(metric.value);
      return acc;
    }, {
      participation: 0,
      learning: 0,
  ...
  aggregateMetricsByUser(metrics: any[]): Record<string, any> {
    ...
    Object.keys(userMetrics).forEach(userId => {
      ...
      user.total = user.participation + user.learning + user.activity + user.networking + user.trade;
    });
  ...
  getLastActivity(userId: string, metrics: any[]): string | undefined {
    ...
    const lastMetric = userMetrics.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0];
