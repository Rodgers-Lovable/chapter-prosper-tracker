...
export const tradesService = {
  ...
  ): Promise<{ data: Trade | null; error: any }> {
    try {
  ...
  async getUserTrades(
    userId: string
  ): Promise<{ data: TradeWithProfiles[] | null; error: any }> {
    try {
      ...
      const { data: tradesData, error } = await supabase
        ...
        .select("*")
        .or(
          `user_id.eq.${userId},source_member_id.eq.${userId},beneficiary_member_id.eq.${userId}`
        )
      ...
      tradesData.forEach((trade) => {
        ...
        if (trade.beneficiary_member_id)
          profileIds.add(trade.beneficiary_member_id);
      });
      ...
      const tradesWithProfiles: TradeWithProfiles[] = tradesData.map(
        (trade) => ({
          ...trade,
          source_member: trade.source_member_id
            ? profilesMap[trade.source_member_id]
            : undefined,
          beneficiary_member: trade.beneficiary_member_id
            ? profilesMap[trade.beneficiary_member_id]
            : undefined,
        })
      );
