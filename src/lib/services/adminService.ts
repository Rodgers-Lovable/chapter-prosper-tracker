...
export const adminService = {
  ...
  async getAdminMetrics(): Promise<AdminMetrics> {
    try {
      const [
        ...
        supabase.from("trades").select("amount").eq("status", "pending"),
      ]);

      const totalRevenue =
        revenueData?.reduce((sum, trade) => sum + Number(trade.amount), 0) || 0;
    ...
    try {
      ...
      const chaptersWithStats = await Promise.all(
        (data || []).map(async (chapter) => {
          ...
          const totalRevenue =
            trades?.reduce((sum, trade) => sum + Number(trade.amount), 0) || 0;
          ...
          return {
            ...chapter,
            leader:
              chapter.leader && chapter.leader.length > 0
                ? chapter.leader[0]
                : null,
  ...
  async getUsers(
    ...
    try {
      ...
      if (filters?.role) {
        query = query.eq(
          "role",
          filters.role as "member" | "chapter_leader" | "administrator"
        );
      }
  ...
  // Create new user
  async createUser(userData: UserDataInterface) {
    try {
  ...
  async getTrades(
    ...
    try {
      ...
      if (filters?.status) {
        query = query.eq(
          "status",
          filters.status as "pending" | "paid" | "invoiced" | "failed"
        );
      }
      if (filters?.chapter_id) {
        query = query.eq("chapter_id", filters.chapter_id);
      ...
      const transformedTrades = (data || []).map((trade) => ({
        ...
        chapter: Array.isArray(trade.chapter)
          ? trade.chapter[0] || null
          : trade.chapter,
        source_member: Array.isArray(trade.source_member)
          ? trade.source_member[0] || null
          : trade.source_member,
        beneficiary_member: Array.isArray(trade.beneficiary_member)
          ? trade.beneficiary_member[0] || null
