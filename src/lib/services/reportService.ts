...
export const reportService = {
  async exportToExcel(
    ...
    // Metrics sheet
    const metricsData = [["Date", "Type", "Value", "Description"]];
    ...
    // Download
    const fileName = `PLANT_Report_${profile.full_name}_${
  ...
  async generateAdminReport(
    ...
    const fileName = `${reportType}_Report_${format(
      startDate,
      "yyyy-MM-dd"
    )}_to_${format(endDate, "yyyy-MM-dd")}.${
    ...
    switch (reportType) {
      case "metrics":
        ...
        break;
      case "trades":
        await this.generateTradesReport(
          startDate,
          endDate,
          exportFormat,
          fileName
      ...
      case "financial":
        ...
        break;
      case "members":
        await this.generateMembersReport(
          startDate,
          endDate,
          exportFormat,
          fileName
        ...
        break;
      case "chapters":
        await this.generateChaptersReport(
          startDate,
          endDate,
          exportFormat,
          fileName
    ...
    await supabase.from("reports_history").insert({
      ...
      report_period: `${format(startDate, "yyyy-MM-dd")} to ${format(
        endDate,
        "yyyy-MM-dd"
      )}`,
      format: exportFormat,
  ...
  async generateTradesReport(
    ...
    // Fetch related data
    const sourceIds = [
      ...new Set(trades?.map((t) => t.source_member_id).filter(Boolean)),
    ];
    const beneficiaryIds = [
      ...new Set(trades?.map((t) => t.beneficiary_member_id).filter(Boolean)),
    ];
    ...
    // Summary
    const totalValue =
      trades?.reduce((sum, t) => sum + Number(t.amount), 0) || 0;
    ...
    trades?.forEach((t) => {
      const source = t.source_member_id
        ? profileMap.get(t.source_member_id)
        : null;
      const beneficiary = t.beneficiary_member_id
        ? profileMap.get(t.beneficiary_member_id)
        : null;
  ...
  async generateMembersReport(
    ...
    // Fetch chapters
    const chapterIds = [
      ...new Set(members?.map((m) => m.chapter_id).filter(Boolean)),
    ];
    const { data: chapters } = await supabase
      .from("chapters")
      .select("id, name")
  ...
  async generateChaptersReport(
    ...
    // Fetch leaders
    const leaderIds = [
      ...new Set(chapters?.map((c) => c.leader_id).filter(Boolean)),
    ];
