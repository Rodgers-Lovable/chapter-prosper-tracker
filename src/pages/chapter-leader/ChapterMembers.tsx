...
const ChapterMembers = () => {
  ...
  const [reminderType, setReminderType] = useState<"metrics" | "payment">(
    "metrics"
  );
  ...
  const handleSendReminder = async () => {
    ...
    try {
      const promises = reminderDialog.members.map((member) =>
        chapterLeaderService.sendMemberReminder(
          member.id,
          reminderType,
          reminderMessage
        )
  ...
  const handleMemberAction = async (
    memberId: string,
    action: "activate" | "deactivate" | "resend_invite"
  ) => {
    try {
      let result;
      switch (action) {
        case "activate":
          result = await chapterLeaderService.updateMemberStatus(
            memberId,
            "active"
          );
          break;
  ...
  const handleBulkAction = async (
    action: "activate" | "deactivate" | "send_reminder"
  ) => {
    ...
    try {
      const promises = selectedMembersList.map((member) =>
        chapterLeaderService.updateMemberStatus(
          member.id,
      ...
      <div className="p-4 md:p-6 space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-success">
                {totalCount > 0
                  ? Math.round(
                      ((totalCount - inactiveCount) / totalCount) * 100
                    )
                  : 0}
                %
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Search and filter chapter members</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <TableBody>
                    {filteredMembers.map((member) => (
                      <TableRow
                        key={member.id}
                        className={
                          selectedMembers.has(member.id) ? "bg-muted/50" : ""
                        }
                      >
                        <TableCell>
                          <Checkbox
                        ...
                        <TableCell>
                          <div>
                            <div className="font-medium">
                              {member.business_name || "Not specified"}
                            </div>
                            <div className="text-sm text-muted-foreground">
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <div className="text-xs text-muted-foreground">
                              P:{member.metrics?.participation || 0} L:
                              {member.metrics?.learning || 0}
                              A:{member.metrics?.activity || 0} N:
                              {member.metrics?.networking || 0}
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {member.isInactive ? (
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleMemberAction(member.id, "activate")
                                  }
                                >
                                  <UserCheck className="mr-2 h-4 w-4" />
                                  Activate Member
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleMemberAction(member.id, "deactivate")
                                  }
                                >
                                  <UserX className="mr-2 h-4 w-4" />
                                  Deactivate Member
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  handleMemberAction(member.id, "resend_invite")
                                }
                              >
        ...
        <Dialog
          ...
          onOpenChange={(open) => setProfileModal({ open })}
        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-3">
                        <div>
                          ...
                          <p className="text-sm">
                            {profileModal.member.full_name || "Not provided"}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-muted-foreground">
                        ...
                        <div>
                          ...
                          <p className="text-sm">
                            {profileModal.member.phone || "Not provided"}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-muted-foreground">
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <p className="text-sm">
                            {profileModal.member.business_name ||
                              "Not provided"}
                          </p>
                        </div>
                        <div>
                          ...
                          <p className="text-sm">
                            {new Date(
                              profileModal.member.created_at
                        ...
                        <div>
                          ...
                          <p className="text-sm">
                            {formatLastActivity(
                              profileModal.member.lastActivity
                            )}
                          </p>
                        </div>
                        <div>
                      ...
                      {[
                        {
                          label: "Participation",
                          value:
                            profileModal.member.metrics?.participation || 0,
                          icon: Users,
                        },
                        {
                          label: "Learning",
                          value: profileModal.member.metrics?.learning || 0,
                          icon: BarChart3,
                        },
                        {
                          label: "Activity",
                          value: profileModal.member.metrics?.activity || 0,
                          icon: TrendingUp,
                        },
                        {
                          label: "Networking",
                          value: profileModal.member.metrics?.networking || 0,
                          icon: Users,
                        },
                        {
                          label: "Trade",
                          value: profileModal.member.metrics?.trade || 0,
                          icon: DollarSign,
                        },
                      ].map((metric) => (
        ...
        <Dialog
          ...
          onOpenChange={(open) => setReminderDialog({ open, members: [] })}
        >
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select
                  value={reminderType}
                  onValueChange={(value) =>
                    setReminderType(value as "metrics" | "payment")
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setReminderDialog({ open: false, members: [] })}
              >
                Cancel
              </Button>
