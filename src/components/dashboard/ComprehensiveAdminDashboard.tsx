  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const completedTrades =
        trades?.filter((trade) => trade.status === "paid").length || 0;
      const pendingTrades =
        trades?.filter(
          (trade) => trade.status === "pending" || trade.status === "invoiced"
        ).length || 0;
      ...
      const paidInvoiceCount =
        invoices?.filter((inv) => inv.paid_at !== null).length || 0;
      const pendingInvoiceCount =
        invoices?.filter((inv) => inv.paid_at === null).length || 0;

      const successfulPayments =
        (paidInvoiceCount / Math.max(totalInvoiceCount, 1)) * 100;
      const failedPayments =
        ((totalInvoiceCount - paidInvoiceCount - pendingInvoiceCount) /
          Math.max(totalInvoiceCount, 1)) *
        100;
      ...
      const averageParticipation =
        metrics && metrics.length > 0
          ? metrics
            ...
            Math.max(
              metrics.filter((m) => m.metric_type === "participation").length,
              1
            )
          : 0;
      ...
      const monthlyGrowth = totalUsers
        ? ((recentUsers || 0) / totalUsers) * 100
        : 0;
      ...
      const topChaptersStats: ChapterStats[] =
        chaptersData
          ?.map((chapter) => {
            const memberCount =
              memberCounts?.filter((m) => m.chapter_id === chapter.id).length ||
              0;
            const chapterTradeList = chapterTrades.filter(
              (trade) => trade.chapter_id === chapter.id
            );
            const monthlyTrades = chapterTradeList.length;
            const revenue = chapterTradeList.reduce(
            ...
            const participation =
              memberCount > 0 ? (monthlyTrades / memberCount) * 100 : 0;
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Complete national overview of PLANT Metrics performance with live
            data
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <CardContent>
            ...
            <div className="flex items-center mt-2">
              ...
              <span className="text-xs text-green-600">
                {(
                  (stats.completedTrades / Math.max(stats.totalTrades, 1)) *
                  100
                ).toFixed(1)}
                % success rate
              </span>
        ...
        <Card className="border-l-4 border-l-orange-500">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats.successfulPayments.toFixed(1)}%
            </div>
            <p className="text-xs text-muted-foreground">
                ...
                <div className="grid grid-cols-4 gap-6 text-center">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-green-600">
                      {chapter.monthlyTrades}
                    </p>
                    <p className="text-xs text-muted-foreground">Trades</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-purple-600">
                      KES {chapter.revenue.toLocaleString()}
                    </p>
                    <p className="text-xs text-muted-foreground">Revenue</p>
                  </div>
                  <div className="flex flex-col items-center">
                    <Progress
                      value={chapter.participation}
                      className="w-20 mb-1"
                    />
                    <p className="text-sm font-medium">
                      {chapter.participation.toFixed(0)}%
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Participation
                    </p>
          ...
          <div className="space-y-3">
            {recentActivity.length > 0 ? (
              recentActivity.map((activity) => (
                <div
                  key={activity.id}
                  className="flex items-center justify-between p-3 bg-muted/50 rounded border-l-4 border-l-blue-500"
                >
