...
serve(async (req) => {
  ...
  try {
    ...
    if (resultCode === 0) {
          ...
          await supabaseClient.functions.invoke('send-notifications', {
            body: {
              ...
              data: {
                ...
                paymentDate: new Date(transactionDate || Date.now()).toLocaleDateString()
              }
            }
          });
      ...
      console.log(`Payment successful for CheckoutRequestID: ${checkoutRequestId}, Receipt: ${mpesaReceiptNumber}`);
    } else {
      console.log(`Payment failed for CheckoutRequestID: ${checkoutRequestId}, Reason: ${resultDescription}`);
    }
