...
interface NotificationRequest {
  type: 'invoice' | 'reminder' | 'payment_confirmation' | 'chapter_announcement';
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Notification batch completed: ${successful} successful, ${failed} failed`);
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
        ...
        <p><strong>Amount:</strong> KES ${Number(data.amount).toLocaleString()}</p>
...
function generateReminderEmail(data: any): string {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Dear ${data.memberName},</p>
      <p>This is a friendly reminder to update your PLANT metrics for this ${data.period}.</p>
      
      <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h3>Your Current Progress</h3>
        <p><strong>Participation:</strong> ${data.currentMetrics?.participation || 0}</p>
        <p><strong>Learning:</strong> ${data.currentMetrics?.learning || 0}</p>
        <p><strong>Activity:</strong> ${data.currentMetrics?.activity || 0}</p>
        <p><strong>Networking:</strong> ${data.currentMetrics?.networking || 0}</p>
      ...
      <p><a href="${data.dashboardUrl}" style="background-color: #2563eb; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px;">Update Metrics Now</a></p>
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background-color: #ecfdf5; padding: 20px; border-radius: 8px; margin: 20px 0;">
        ...
        <p><strong>Invoice Number:</strong> ${data.invoiceNumber}</p>
        <p><strong>Amount Paid:</strong> KES ${Number(data.amount).toLocaleString()}</p>
...
function generateChapterAnnouncementEmail(data: any): String {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p style="margin-top: 30px;">Best regards,<br>${data.chapterName} Chapter</p>
    </div>
  `;
}